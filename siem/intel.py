...
def validate(kind,value):
    ...
    return value
def list_items(q='',kind='',verdict='',page=1,size=50):
    clauses=['1=1']; params=[]
    ...
    with store.connection() as db:
        ...
        rows=db.execute('SELECT * FROM intelligence WHERE '+where+' ORDER BY last_seen DESC LIMIT ? OFFSET ?',params+[size,(page-1)*size]).fetchall()
        return {'total':total,'items':[dict(r) for r in rows]}
def upsert(kind,value,source='manual',confidence=0,verdict='unknown',tags=None,notes=''):
    ...
    with store.connection() as db:
        ...
        store.audit('intel.upsert',f'{kind}:{value} ({source})',db=db)
...
def remove(item_id):
    with store.connection() as db:
        ...
        store.audit('intel.delete',f'#{item_id}',db=db)
