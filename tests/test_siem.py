...
class SIEMTests(unittest.TestCase):
    ...
    def test_api_auth_validation_export_and_incident_state(self):
        ...
        with TestClient(app) as client:
            ...
            waf_rules=client.get('/api/waf-rules').json()
            self.assertEqual(waf_rules['engine'],'OWASP Core Rule Set')
            self.assertTrue(any(r['rule_range']=='942xxx' for r in waf_rules['families']))
